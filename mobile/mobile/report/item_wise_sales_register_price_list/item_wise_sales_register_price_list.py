...
def get_price_lists_from_filter(filters):
	import json

	price_lists = filters.get("price_list")
	if not price_lists:
		return []
	...
	return [p.get("value", p) if isinstance(p, dict) else p for p in price_lists]


def get_item_prices(item_list, price_lists):
	if not item_list or not price_lists:
		return {}

	item_codes = list(set(d.item_code for d in item_list))
	...
	prices = (
		...
		.where(ip.price_list.isin(price_lists))
		.where(ip.selling == 1)
		.run(as_dict=True)
	)
	...
	price_map = {}
	for p in prices:
		price_map.setdefault(p.item_code, {})[p.price_list] = p.price_list_rate

	return price_map


def get_summarised_data(data, price_lists, tax_columns):
	summary = {}
	for row in data:
		if not row.get("item_code"):
			continue
		key = (row.get("item_code"), row.get("customer"))
		if key not in summary:
			...
			for pl in price_lists:
				summary[key][f"price_{frappe.scrub(pl)}"] = 0.0
			for tax in tax_columns:
				summary[key][f"{tax}_amount"] = 0.0

		summary[key]["stock_qty"] += flt(row.get("stock_qty"))
		summary[key]["qty"] += flt(row.get("qty"))
		summary[key]["amount"] += flt(row.get("amount"))
		summary[key]["total_tax"] += flt(row.get("total_tax"))
		summary[key]["total_other_charges"] += flt(row.get("total_other_charges"))
		summary[key]["total"] += flt(row.get("total"))

		for pl in price_lists:
			field = f"price_{frappe.scrub(pl)}"
...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	if filters.get("summarise_data"):
		_summarise_skip = {
			"description", "invoice", "posting_date", "debit_to", "mode_of_payment",
			"sales_order", "delivery_note", "income_account", "cost_center", "project", "rate",
			"customer_name",
		}
		columns = [c for c in columns if c.get("fieldname") not in _summarise_skip]

	company_currency = frappe.get_cached_value("Company", filters.get("company"), "default_currency")

	item_list = get_items(filters, additional_table_columns, additional_conditions)

	price_lists = get_price_lists_from_filter(filters)
	item_prices = {}
	if price_lists:
		item_prices = get_item_prices(item_list, price_lists)
		amount_index = next((i for i, c in enumerate(columns) if c.get("fieldname") == "amount"), len(columns) - 1)
		for i, pl in enumerate(price_lists):
			columns.insert(
				amount_index + 1 + i,
				{
					"label": _(pl),
					"fieldname": f"price_{frappe.scrub(pl)}",
					"fieldtype": "Currency",
					"options": "currency",
					"width": 120,
				},
	...
	for d in item_list:
		...
		for pl in price_lists:
			pl_rate = item_prices.get(d.item_code, {}).get(pl, 0)
			row[f"price_{frappe.scrub(pl)}"] = flt(pl_rate) * flt(d.qty)
	...
	if filters.get("summarise_data"):
		data = get_summarised_data(data, price_lists, tax_columns if item_list else [])
		skip_total_row = 0
